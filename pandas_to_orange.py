...
#         return ContinuousVariable(str(d.name), number_of_decimals=0)
#     else:
#         t = d.unique()
#         if discrete or len(t) < len(d) / 2:
#             t.sort()
#             return DiscreteVariable(str(d.name), values=list(t.astype("str")))
#         else:
#             return StringVariable(str(d.name))
#
#
# def df2domain(df):
#     featurelist = [series2descriptor(df.icol(col)) for col in range(len(df.columns))]
...
#     ttables = [series2table(df.icol(i), tdomain[i]) for i in range(len(df.columns))]
#     return Orange.data.Table(np.reshape(ttables, (len(ttables), -1)))
#
#
...
#     series = [column2df(tab.select(i)) for i in range(len(tab.domain))]
...
def df2domain(df):
    featurelist = [series2descriptor(df.iloc[:,col]) for col in xrange(len(df.columns))]
    return Orange.data.Domain(featurelist)
...
def df2table(df):
    ...
    tdomain = df2domain(df)
    ttables = [series2table(df.iloc[:,i], tdomain[i]) for i in xrange(len(df.columns))]
